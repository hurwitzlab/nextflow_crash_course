...
If you try running `fastqc_trimmomatic_params.nf` you will observe **that the output directory is not called `results/`, but rather it is called `params.outdir/`**. Why is that? 
...
2. Add `params.input` and `params.outdir` to a `nextflow.config` file (you won't use genome yet today — it's there because you'll need it for the capstone assembly step this afternoon).
