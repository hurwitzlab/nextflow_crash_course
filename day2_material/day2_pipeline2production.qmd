...
::: {.callout-note}
Notice the default value itself also changes here, from yesterday's local toy files to the shared training dataset on the cluster (`/gpfs_backup/bioinfo_data/training_data/nextflow_crash_course`) — this is the real dataset you'll run the rest of today's pipeline against, now that it's becoming something you'd actually reuse rather than a one-off script.
:::
...
If you try running `fastqc_trimmomatic_params.nf` you will observe **that the output directory is not called `results/`, but rather it is called `params.outdir/`**. But we created the parameter and we gave it the value results, so why is that? 
> Here `publishDir 'params.outdir/qc_raw'` created a folder with that literal name. In Groovy (the language underlying Nextflow's grammar), **single-quoted strings are never interpolated**. 
> **The fix** is a double-quoted string: `publishDir "${params.outdir}/qc_raw"`.
:::
...
## 3.`getManifest()`: naming every output once

Right now, every `publishDir` you write picks its own path — `results/fastqc`,
`results/trimmed`, whatever seemed reasonable at the time. That's fine for
one pipeline run by one person. It breaks down the moment you have several
pipelines, run by several people, whose outputs need to sit next to each
other in a shared location — say, on `/rs1`, where everyone's assemblies,
QC reports, and annotations are meant to accumulate into one browsable,
queryable collection instead of a pile of one-off folder names.

The fix is to stop deciding paths process-by-process and declare them all
once, in a single Groovy map:
...
def getManifest() {
    [
        ...
        qc_html     : "log/fastp_log.html",
        qc_json     : "log/fastp_log.json",
    ]
}
```

> You can see more examples of `getManifest()` in the `manifests/` folder under day2_material

Every process's `publishDir` then references a key from this map instead of
a hand-typed string. Nothing here is special Nextflow syntax — `getManifest()`
is a plain Groovy function you write, returning a plain map. The value is
entirely in the convention: **one place decides where every output goes, for
every pipeline that adopts it.**

::: {.callout-important}
This is *not* Nextflow's built-in `manifest {}` block in `nextflow.config`
(that one holds pipeline metadata — name, author, version — read by
`nextflow pull`/`nextflow run <repo>`). This is a project convention that
happens to reuse the word "manifest." Different mechanism, different
purpose — don't confuse the two if you go looking at Nextflow's docs.
:::

**Why bother?** Two payoffs, both downstream of the same fact — the path for
any given output is now *known in advance*, not something you have to run
the pipeline and go look for:

- **A shared data lake.** If every pipeline that touches `/rs1` writes
  `contigs.fa` to the same relative spot, outputs from different projects,
  different people, and different runs land in a predictable structure —
  the collection becomes something you can query across, instead of
  something you have to relearn per project.
- **Machine-readable by default.** A script — or an AI agent — that needs
  to find "the assembly output" doesn't have to parse your Groovy code to
  figure out where it went. It reads the manifest. This is also the
  foundation for emitting a `manifest.json` alongside your results at the
  end of a run: a small file that says exactly what was produced and where,
  so any downstream tool can consume your pipeline's outputs without
  knowing anything about how the pipeline itself works.

::: {.callout-tip}
We won't wire `getManifest()` into `publishDir` directives in this workshop
— that's a natural next step once you're writing pipelines of your own, past
the point where 5–6 processes fit comfortably in your head. The pattern is
worth knowing now so you recognize it when your outputs start needing to
live somewhere other than your own laptop.
:::



## 4. Overriding from the command line
...
```bash
nextflow run pipeline.nf --input '/gpfs_backup/bioinfo_data/training_data/nextflow_crash_course/sample_2_*.fastq.gz' --outdir results_sample2
```
...
## 5. `-resume`: don't redo finished work
...
```
executor >  local (2)
[a1/2b3c4d] fastqc_raw (1)   | 5 of 5, cached: 5 ✔
[e5/6f7a8b] trim (1)         | 5 of 5, cached: 1 ✔
```
...
## 6. Profiles: one pipeline, swappable environments
...
```bash
nextflow run pipeline.nf -profile hazel
```


## 7. Exercise: parameterize the pipeline

Using your Day 1 3-step pipeline (fastqc_raw → trim → fastqc_trimmed) as a starting point:

1. Replace the hardcoded `Channel.fromPath('data/sample_*_*.fastq')` with `Channel.fromPath(params.input)`, and point its default at the shared training dataset (`/gpfs_backup/bioinfo_data/training_data/nextflow_crash_course/sample_*_*.fastq.gz`) instead of yesterday's local toy files.
2. Add `params.input` and `params.outdir` to a `nextflow.config` file.
...
5. Add a second, empty profile (`test {}` alongside `hazel {}`) to nextflow.config, and run once with `-profile test` just to confirm the flag is accepted — nothing will look different yet, and that's the point.
...
::: {.callout-important}
## Note on `${task.cpus}`
We've gone a bit further in our parameterization: the scripts now use `${task.cpus}` instead of a hardcoded thread count. `task.cpus` is the value of the `cpus` directive for *that task*, so it comes from your config, not from the script.

With `-profile hazel`, the single line `process.cpus = 2` does two jobs at once. It asks SLURM for 2 cores, and it passes `-t 2` to FastQC and `-threads 2` to Trimmomatic. What you request and what the tool uses can't drift apart.

Without a profile that sets `cpus` (a plain `nextflow run main.nf`, or `-profile test`), `task.cpus` falls back to **1**, and the same script runs single-threaded. The code is the same and only the resources change.

To give one process more cores, change the config, not the script:

```groovy
process {
    withName: 'trim' { cpus = 4 }
}
```

`withName` settings take priority over the generic `process.cpus`. `resourceLimits` caps whatever you request, so asking for more than 24 CPUs gets trimmed to 24 instead of leaving a job that never schedules.
:::
:::
...
```
my_pipeline/
├── main.nf          ← just the workflow block + includes
├── nextflow.config
└── modules/
    ├── fastqc.nf
    ├── trim.nf
...
## 4. `utils/` and `bin/`: helpers that aren't processes

Everything in `modules/` so far has been a **process** — a unit of work with
its own `input:`, `output:`, and `script:` block. Not everything you want to
reuse fits that shape. Two common cases, two different homes:

**`utils/` — Groovy helper functions.** Plain logic, `include`d the same way
as a process:

```groovy
// utils/archive.nf
def untarCmd(String archivePath) {
    "tar -xzf ${archivePath}"
}
```
...
```groovy
// main.nf
include { untarCmd } from './utils/archive.nf'
...
process extract {
    ...
    script:
    "${untarCmd(archive.name)}"
}
```

**`bin/` — standalone scripts, any language.** If a pipeline author has
written a Python script — say, `retrieve_fasta.py`, for pulling a reference
sequence from a database — it doesn't go in `utils/`. It goes in a `bin/`
directory at the pipeline root. Nextflow automatically adds `bin/` to every
process's `PATH`, so the script is callable **by name**, with no `include`
and no full path:
...
process fetch_reference {
    ...
    script:
    """
    retrieve_fasta.py --accession GCF_000001405.40 -o reference.fa
    """
}
```

(The script needs its executable bit set — `chmod +x bin/retrieve_fasta.py`
— and a shebang line, same as running it standalone.)

::: {.callout-tip}
Same scoping rule as `modules/`: helpers used by one pipeline stay nested
inside it (`utils/`, `bin/`); helpers useful across several pipelines move
up into a shared location each pipeline's `include` or `PATH` can reach.
:::

::: {.callout-tip}
## A note on pipeline layout

As your pipeline grows past one `.nf` file, where things live starts to
matter. A common convention:
```
my_pipeline/
├── main.nf
├── nextflow.config
...
Everything belonging to *this* pipeline — its entry point, its config, and
any process or helper only *this* pipeline uses — lives under one named
directory. That's the unit you'd hand to a collaborator, or point `nextflow
run` at.

If a module or helper function is useful across *several* pipelines —
say, a `fastqc.nf` module every pipeline in your group reuses — it moves up
a level, out of any one pipeline's directory, into a shared location that
each pipeline's `include` statement points to:

```groovy
// inside my_pipeline/main.nf
include { fastqc } from '../shared_modules/fastqc.nf'
```

The rule of thumb: **scope decides location.** Pipeline-specific stays
nested inside the pipeline; anything meant to be reused moves up and out.
This is the same instinct behind the manifest convention from earlier —
predictable structure is what makes a pipeline (and its outputs) something
other people, or other pipelines, can actually build on.
:::

## 5. Exercise: refactor into modules
...
You already met `profiles` in §2 as a way to swap settings without touching your pipeline code — a `test` profile with small inputs, a `hazel` profile with executor settings. Container engines slot into the exact same mechanism. `docker` and `singularity` (or `apptainer`) are just profile names that flip a container engine on:
...
```
3. **Bind paths that aren't auto-mounted.** With `autoMounts = true`, Nextflow binds the directories of each task's declared inputs, and Apptainer binds your home directory and the current directory by default. That's not everything. If a tool reaches `/rs1` (our shared storage) through a path Nextflow doesn't know about, like a database path passed as a string or a symlink pointing into `/rs1`, the container can't see it. If a file "isn't found" inside the container even though it exists on disk, that's almost always a missing bind, not a missing file.

Because `/rs1` is a Hazel detail, not a FastQC detail, the bind goes in the profile, not the process:

```groovy
apptainer {
    enabled    = true
    autoMounts = true
    runOptions = '--bind /rs1:/rs1'
}
```

The process itself only names its image:
...
::: {.callout-tip collapse="true"}
## Why single quotes on `runOptions`?
The quote style decides **who** expands `${...}`, and **when**.

**Single quotes are literal text.** Groovy doesn't interpret anything inside them, so `'--bind ${TMPDIR:-/tmp}'` is kept exactly as typed. Nextflow copies that text into the job script (`.command.run`). When the job starts on the compute node, **bash** expands `${TMPDIR:-/tmp}`: "use `$TMPDIR`, or `/tmp` if it's unset."

**Double quotes are Groovy interpolation.** In `"..."`, Groovy treats every `${...}` as Groovy code and fills it in **when the config is read**. That happens once, on the node where you ran `nextflow run`, before any job exists. `"--bind ${TMPDIR:-/tmp}"` breaks in two ways:

1. `TMPDIR:-/tmp` is bash syntax, not Groovy, so the config fails to parse.
2. Even the Groovy version, `"--bind ${System.getenv('TMPDIR')}"`, binds the wrong directory. It captures the launch node's `$TMPDIR`, but SLURM gives each job its own `$TMPDIR` on the compute node.

Compare `cacheDir`, a few lines up, which uses double quotes on purpose. Nextflow uses that directory itself on the launch node, so reading the environment once at launch is exactly right.
:::
...
You could have created two profiles, one with the resource information and one with the apptainer information and run nextflow with `nextflow run main.nf -profile hazel,apptainer...` to combine both — profiles stack, so your container settings from §4 and your scheduler settings here apply together without redefining anything.
...
- **From an interactive session** (the same `srun` session from Day 1, §2 — same flags, no need to redo that here): you type `nextflow run main.nf -profile hazel` directly. Nextflow's own head process runs live in your terminal, submitting each task as a separate SLURM job as the pipeline progresses. Good for development and debugging — you see output immediately, `-resume` is easy to reach for.
...
**Your 1st task:** add a `withName` block for your assembly process that requests more CPUs and memory than FastQC's defaults, then re-run with `-profile hazel` and confirm in the HAZEL job log (`sacct -j <jobid>`) that the assembly task actually requested the resources you gave it.
...
**Your 2nd task:** inside the `hazel` profile you wrote in §5.2, add a `withName` block for your assembly process that requests more CPUs and memory than FastQC's defaults, then re-run with `-profile hazel` and confirm in the SLURM job log (`sacct -j <jobid>`) that the assembly task actually requested the resources you gave it.
...
::: {.callout-note}
### Why `withName` lives inside the `hazel` profile, not outside it
...
Nesting `withName` inside `profiles { hazel { ... } }` scopes it correctly: these per-process resource rules only take effect when you actually run with `-profile hazel`. Anything SLURM-specific — resource sizing included — belongs inside the profile that turns SLURM on.
:::
:::
...
```groovy
   ...
   workflow {
       main:
       reads_ch = Channel.fromPath('/gpfs_backup/bioinfo_data/training_data/nextflow_crash_course/sample_*_*.fastq.gz')
...
4. **Run on the `hazel` profile**:
```bash
   nextflow run main.nf -profile hazel
```
6. **Give `assembly` its own resource block** (you should already have done this in 5.5) in the `hazel` profile (§5.5) — assembly is memory-hungry, more so than `fastqc` or `trim`:
```groovy
   withName: 'assembly' {
       cpus   = 8
       memory = 32.GB
...
> Note: Think about if BWA + SAMtools should be a subworkflow since you will probably use them together a lot 